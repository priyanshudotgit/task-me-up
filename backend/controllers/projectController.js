...
export const createProject = async (req, res) => {
    try {
        ...
        if(!workspace.members.some((member) => member.userId === userId && member.role === "ADMIN")){
            return res.status(403).json({message: "Unauthorized to create workspace!"});
        }
        ...
        const projectWithMembers = await prisma.project.findUnique({
            ...
            include: {
                ...
                tasks: {include: {asignee: true, comments: {include: {user: true}}}},
                owner: true
            }
        })
    ...
    try {
        ...
        if(!workspace.members.some((member) => member.userId === userId && member.role === "ADMIN")){
            ...
            if(!project){
                ...
                return res.status(403).json({message: "Unauthorized to update project in this workspace!"});
            }
        }
...
export const addMember = async (req, res) => {
    try {
        ...
        if(project.team_lead !== userId){
            return res.status(404).json({message: "Unauthorized to add members!"});
        }
